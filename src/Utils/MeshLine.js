...
MeshLine.prototype.toData = function(){
	return {
...
MeshLine.prototype.process = function () {
	...
	if (!this.attributes) {
		this.attributes = {
			...
			number:new THREE.BufferAttribute(new Float32Array(this.numbers), 1),
		...
		this.attributes.number.copyArray (new Float32Array(this.numbers));
		this.attributes.number.needsUpdate = true;
	}
	...
	this.geometry.setAttribute("number", this.attributes.number);
...
THREE.ShaderChunk["meshline_vert"] = [
	...
	// 根据投影角度动态改变当前点应该有的宽度，这个算法很迷，比平面几何法牛逼有空研究下,fuck没啥用，这个也还是没做到线条等宽啊，得给他加个三角函数算法
	"    float pixelWidthRatio = 1. / (resolution.x * projectionMatrix[0][0]);",
	"",
	"    vColor = vec4( color, opacity );",
	`	
		if( lineLength>1.0){
			vUV = vec2((number - start)/lineLength,uv.y);
		} else{ 
			vUV = vec2(0,uv.y);
		}
	`,
